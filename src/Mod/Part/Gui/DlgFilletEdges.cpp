                                        ...
                                        const QModelIndex &index) const
{
    ...
    //QString value = QString("%1").arg(spinBox->value(),0,'f',2);
...
void DlgFilletEdges::onSelectEdge(const QString& subelement, int type)
{
    ...
    for (int i=0; i<model->rowCount(); ++i) {
        ...
        QString name = QString("Edge%1").arg(id);
        if (name == subelement) {
...
void DlgFilletEdges::onDeleteObject(const App::DocumentObject& obj)
{
    if (d->fillet == &obj) {
        ...
        QString shape = QString(obj.getNameInDocument());
...
void DlgFilletEdges::findShapes()
{
    ...
    for (std::vector<App::DocumentObject*>::iterator it = objs.begin(); it!=objs.end(); ++it, ++index) {
        ui->shapeObject->addItem(QString((*it)->Label.getValue()));
        ui->shapeObject->setItemData(index, QString((*it)->getNameInDocument()));
        if (current_index == 0) {
...
bool DlgFilletEdges::accept()
{
    ...
    type = QString("Part::%1").arg(QString(fillet.c_str()));

    if (d->fillet)
        name = QString(d->fillet->getNameInDocument());
    else
        name = QString(activeDoc->getUniqueObjectName(fillet.c_str()).c_str());
