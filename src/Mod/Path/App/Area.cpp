...
static void addG0(bool verbose, Toolpath &path,
        ...
        double f, double &last_f) //# maybe need max feedrate?
{
    gp_Pnt pt(last);
    if(retraction-(last.*getter)() > Precision::Confusion()) {
        (pt.*setter)(retraction);
        addGCode(verbose,path,last,pt,"G0"); //# move to retraction height
    }
        last = pt;
        pt = next;
        (pt.*setter)(retraction);
        addGCode(verbose,path,last,pt,"G0"); //# move to rest point coordinates

    if(retraction-(next.*getter)() > Precision::Confusion()) {
    	addGCode(verbose,path,pt,next,"G1");
    }
/* // under construction
...
#include <streambuf>
#include <string>
void PathSaveSTEP(int i, std::list<TopoDS_Shape> &shapes){
	// https://www.opencascade.com/doc/occt-7.0.0/overview/html/occt_user_guides__step.html#occt_step_3
	STEPControl_Writer writer;
	STEPControl_StepModelType mode = STEPControl_GeometricCurveSet;
//	int i = 0;
        for(auto &shape : shapes) {
	stringstream b;
	b << "/home/a/shape" << i << ".stp";
	string filename = b.str();
    std::cout << filename << "\n";
	writer.Transfer(shape,mode);//IFSelect_ReturnStatus stat1 = 
	writer.Write(filename.c_str());//IFSelect_ReturnStatus stat2 = 
	i++;addG0
        }
}
*/
...
{
//    Base::Console().Log("toPath\n");
//    PathSaveSTEP(0,(std::list<TopoDS_Shape> &)shapes);
...
//    PathSaveSTEP(10,(std::list<TopoDS_Shape> &)wires);
...
pstart = gp_Pnt(0,0,0);
    ...
    gp_Pnt plast;//,p
    plast = pstart;
    cout << "pstart: X " << plast.X() << " Y " << plast.Y() << " Z " << plast.Z() << endl;
    // pstart: X 73.0159 Y 120.165 Z 5 <<<!!! unable correct addG0()
...
//#            const auto &edge = xp.Current();
        for (int count = 0; count < numEdges; count++ ){
            ...
            BRepAdaptor_Curve curve(edge);
            //bool reversed = (edge.Orientation()==TopAbs_REVERSED);
            //plast = curve.Value(reversed?curve.FirstParameter():curve.LastParameter());
            a = curve.Value(curve.FirstParameter());
            b = curve.Value(curve.LastParameter());
            bool reversed = false;
            if(plast.SquareDistance(a)>plast.SquareDistance(b)) {
                std::swap(a,b);
                reversed = true;
            }
            ...
            case GeomAbs_Line: {
                cout << "Line" << endl;
                if(a.SquareDistance(plast)>threshold)
                	addG0(verbose,path,plast,a,getter,setter,retraction,resume_height,vf,cur_f);
                plast = a;
                /**/    //cout << "segmentation:" << segmentation << endl; //
                ...
                if(segmentation > Precision::Confusion()) {
                    GCPnts_UniformAbscissa discretizer(curve, segmentation,
                            curve.FirstParameter(), curve.LastParameter()); //a, b
                    cout << "discretizer.IsDone()" << discretizer.IsDone() << endl;
                    cout << "discretizer.NbPoints()" << discretizer.NbPoints() << endl;
                ...
                addG1(verbose,path,plast,b,nf,cur_f);
                plast = b;
                break;
            } case GeomAbs_Circle:{
            	Base::Console().Log("Circle\n");
                   ...
                   (arcPlane==currentArcPlane || arc_plane==ArcPlaneVariable))
                {
                    ...
                    /**/if(segmentation > Precision::Confusion()) {
                        ...
                        cout << "Circle discretizer.IsDone()" << discretizer.IsDone() << endl;
                        cout << "Circle discretizer.NbPoints()" << discretizer.NbPoints() << endl;
                        if (discretizer.IsDone () && discretizer.NbPoints () > 2) {
                            int nbPoints = discretizer.NbPoints ();
                            if(reversed) {
                                for (int i=nbPoints-1; i>=1; --i) {
                    ...
                    addGArc(verbose,abs_center,path,plast,b,center,clockwise,nf,cur_f);
                    plast=b;
                    break;
                }
 ...
 */
            default: {
            	cout << "GeomAbs OtherCurve" << endl;
            	/*
            	 // here deflection < deflection in discretize(edge,deflection)
                if(a.SquareDistance(plast)>threshold)
                	addG0(verbose,path,plast,a,getter,setter,retraction,resume_height,vf,cur_f);
                plast = a;
            	if(deflection > Precision::Confusion()) {
                    GCPnts_UniformAbscissa discretizer(curve, deflection,
                            curve.FirstParameter(), curve.LastParameter()); //a, b
                    cout << "discretizer.IsDone()" << discretizer.IsDone() << endl;
                    cout << "discretizer.NbPoints()" << discretizer.NbPoints() << endl;
                    if (discretizer.IsDone () && discretizer.NbPoints () > 2) {
            	...
            	*/
                const auto &pts = discretize(edge,deflection); // why too much memory?
                if(pts.size()>0){
    cout << "a: X " << a.X() << " Y " << a.Y() << " Z " << a.Z() << endl;
    cout << "b: X " << b.X() << " Y " << b.Y() << " Z " << b.Z() << endl;
    cout << "pts[0]: X " << pts[0].X() << " Y " << pts[0].Y() << " Z " << pts[0].Z() << endl;
                    auto &pt = pts[0];
                    if(pt.SquareDistance(plast)>threshold)
                    	addG0(verbose,path,plast,pt,getter,setter,retraction,resume_height,vf,cur_f);
                    plast = pt;
                }
    ...
    if(plast.SquareDistance(pstart)>threshold)
        addG0(verbose,path,plast,pstart,getter,setter,retraction,resume_height,vf,cur_f);
}
