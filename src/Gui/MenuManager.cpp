    ...
    for (QList<MenuItem*>::ConstIterator it = items.begin(); it != items.end(); ++it)
    {
        ...
        if (!action) {
            ...
            if ((*it)->command() == "Separator") {
                ...
                action->setObjectName(QString("Separator"));
            }
            else {
                // create a new menu
                ...
                menu->setObjectName(QString(menuName.c_str()));
                action->setObjectName(QString(menuName.c_str()));
            }

            // set the menu user data
            action->setData(QString((*it)->command().c_str()));
        }
        else {
...
void MenuManager::setup(MenuItem* item, QMenu* menu) const
{
    ...
    for (QList<MenuItem*>::ConstIterator it = items.begin(); it != items.end(); ++it) {
        ...
        QList<QAction*> used_actions = findActions(actions, QString((*it)->command().c_str()));
        if (used_actions.isEmpty()) {
            if ((*it)->command() == "Separator") {
                QAction* action = menu->addSeparator();
                action->setObjectName(QString("Separator"));
                // set the menu user data
                action->setData(QString("Separator"));
                ...
                if ((*it)->hasItems()) {
                    ...
                    submenu->setObjectName(QString((*it)->command().c_str()));
                    action->setObjectName(QString((*it)->command().c_str()));
                    // set the menu user data
                    action->setData(QString((*it)->command().c_str()));
                    used_actions.append(action);
                }
                    ...
                    if (mgr.addTo((*it)->command().c_str(), menu)) {
                        ...
                        for (int i=count; i < acts.count(); i++) {
                            QAction* a = acts[i];
                            // set the menu user data
                            a->setData(QString((*it)->command().c_str()));
