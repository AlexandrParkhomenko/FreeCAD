...
QString UnitsSchemaImperial1::schemaTranslate(const Quantity &quant, double &factor, QString &unitString)
{
    ...
    if (unit == Unit::Length) {  // Length handling ============================
        if (UnitValue < 0.00000254) {// smaller then 0.001 thou -> inch and scientific notation
            ...
            unitString = QString("yd");
            factor = 914.4;
        }
        else if(UnitValue < 1609344000.0) {
            unitString = QString("mi");
            factor = 1609344.0;
        }
...
QString UnitsSchemaImperialBuilding::schemaTranslate(const Quantity &quant, double &factor, QString &unitString)
{
    ...
    if (unit == Unit::Length) {
        ...
        // Done!
        return QString(output.str().c_str());
    }
    else if (unit == Unit::Area) {
        unitString = QString("sqft");
        factor = 92903.04;
    }
    else if (unit == Unit::Volume) {
        unitString = QString("cuft");
        factor = 28316846.592;
    }
    else if (unit == Unit::Velocity) {
        unitString = QString("in/min");
...
//    double UnitValue = std::abs(quant.getValue());
    ...
    // now do special treatment on all cases seems necessary:
    if (unit == Unit::Length) {  // Length handling ============================
            unitString = QString("ft");  //always ft
            factor = 304.8;      //12 * 25.4
    }
    else if (unit == Unit::Area) {
        unitString = QString("ft^2");    //always sq.ft
        factor = 92903.04;
    }
    else if (unit == Unit::Volume) {
        unitString = QString("ft^3");    //always cu. ft
        factor = 28316846.592;
    }
    else if (unit == Unit::Mass) {
        unitString = QString("lb");     //always lbs. 
        factor = 0.45359237;
    }
    else if (unit == Unit::Pressure) {
            unitString = QString("psi");
        ...
        unitString = QString("mph");
        factor =  0.002235598;                         //1mm/sec => mph
    }
    // this schema expresses angles in degrees + minutes + seconds
    else if (unit == Unit::Angle) {
        unitString = QString("deg");
        QString degreeString = QString("\xC2\xB0");          //degree symbol
        QString minuteString = QString("\xE2\x80\xB2");      //prime symbol
        QString secondString = QString("\xE2\x80\xB3");      //double prime symbol
